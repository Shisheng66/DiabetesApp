...
class _DietScreenState extends State<DietScreen> {
  ...
  Future<int> _seedFoodLibrary() async {
    try {
      ...
      final exists = _list(
        res,
      ).map((e) => '${_map(e)['name']}'.toLowerCase().trim()).toSet();
      var added = 0;
  ...
  Future<void> _showAddRecord({
    ...
    var selectedFood = presetFoodId ?? (_foods.first['id'] as int?);
    final amountCtrl = TextEditingController(
      text: presetAmount?.toStringAsFixed(0) ?? '',
    );
    final remarkCtrl = TextEditingController(text: presetRemark ?? '');
              ...
              FilledButton(
                onPressed: () async {
                  ...
                  try {
                    ...
                    await _loadAll();
                    if (!mounted) return;
                    AppToast.success(context, '饮食记录添加成功');
                  } on ApiException catch (e) {
                    if (!ctx.mounted) return;
                    ScaffoldMessenger.of(
                      ctx,
